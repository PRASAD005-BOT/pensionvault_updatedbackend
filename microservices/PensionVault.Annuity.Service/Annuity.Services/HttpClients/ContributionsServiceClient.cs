...
public record LocalContribution(string Period);
...
public class ContributionsServiceClient
{
    ...
    public async Task<List<LocalContribution>> GetMemberContributionsAsync(Guid memberId)
    {
        ApplyAuthHeader();
        var response = await _httpClient.GetAsync($"api/remittances/member/{memberId}");
        if (response.StatusCode == System.Net.HttpStatusCode.NotFound) return new List<LocalContribution>();
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<List<LocalContribution>>() ?? new List<LocalContribution>();
    }
}
