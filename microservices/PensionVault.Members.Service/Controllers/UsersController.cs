...
public class UsersController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAll([FromServices] PensionVault.Infrastructure.Data.MembersDbContext context)
    {
        var users = await Microsoft.EntityFrameworkCore.EntityFrameworkQueryableExtensions.ToListAsync(
            Microsoft.EntityFrameworkCore.EntityFrameworkQueryableExtensions.Include(context.Users, u => u.Member)
        );
    ...
    [HttpGet("{id:guid}")]
    [Authorize]
    public async Task<IActionResult> GetUserById(Guid id, [FromServices] PensionVault.Domain.Interfaces.IUserRepository userRepo)
    {
    ...
    [Authorize]
    public async Task<IActionResult> GetByRole(string role, [FromServices] PensionVault.Domain.Interfaces.IUserRepository userRepo)
    {
        if (!Enum.TryParse<PensionVault.Domain.Enums.UserRole>(role, true, out var parsedRole))
