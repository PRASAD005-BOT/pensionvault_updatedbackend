...
public class HttpContributionRepository : BaseHttpRepository, IContributionRepository
{
    ...
    public async Task<List<ContributionRemittance>> GetByEmployerAsync(Guid employerId)
        => await GetAsync<List<ContributionRemittance>>($"api/remittances/employer/{employerId}") ?? new List<ContributionRemittance>();
